...
A request sets `reasoning.effort` to one of these seven values. `max` is a first-class tier above
...
- **Requesting a positive effort** (`minimal`..`max`): resolves to the highest declared level that
...
- **`budgets`** — only for the `*_budget` kinds: a map from effort level to a token budget
  ...
  A budget-based spec that declares `max` must provide its budget explicitly; the gateway never invents
  a portable token threshold for a provider-defined maximum tier.
- **`upstreamEffortMap`** — only for synonymous provider labels whose spelling differs from the
  canonical name. Keys are always canonical; values are the provider's native label. It must never
  collapse distinct semantic tiers: mapping `xhigh` to `max` is invalid. Omitted levels pass through
  unchanged (canonical name == upstream name).

...
```text
max per Public Model pool = 1 + numRetries
max for primary + all fallbacks = maxAttemptsPerRequest
```

The router makes a first pass over all eligible deployments before starting the second, and so on.
It advances to the next fallback when the current pool reaches its budget. Execution ends immediately
when one succeeds, and the full-chain budget prevents a long fallback list from amplifying an outage.
`maxAttemptsPerRequest` must be at least `numRetries + 1`, so it never truncates the configured retry
count for the primary pool.
...
Example with `numRetries = 2` and a sufficiently large request budget:

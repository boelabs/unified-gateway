...
The Vercel catalog document is intentionally empty: Vercel's `creator/model` ids delegate to the
matching reviewed first-party catalog at runtime. Models from creators without a built-in adapter use
an inline `catalogEntry` on their deployment.

...
| `pricing` | no | `{ inputCentsPerMTokens?, outputCentsPerMTokens?, cacheReadCentsPerMTokens?, cacheWriteCentsPerMTokens?, searchUnitCents?, audioInputCentsPerMinute? }`. |
...
Azure transcription is created under `azureopenai`; `upstreamModel` is the **deployment name** and
`baseUrl` is either the resource endpoint or its full `/openai/v1` base. The adapter keeps text and
Realtime on v1 but sends bounded files to Azure's separate, deployment-addressed audio data plane.
Its documented `2025-04-01-preview` version is the default; set credential `apiVersion` only when the
Azure resource requires a different audio API version:
...
For a live model, register a deployment whose catalog entry exposes `audio.transcribe.live`, then
connect to `GET /v1/realtime?model=<publicModel>&intent=transcription` with a WebSocket client. Live
sessions use their own operation profile and never share eligibility with bounded file requests by
accident.
...
- The semantics of pool-wide retries, `reason`, and chain lifecycle are in [fallbacks](/docs/fallbacks).

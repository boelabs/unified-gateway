...
knob (`reasoning.effort`); the gateway snaps it to `levels` and translates it to the upstream. Requests
are **clamped, never rejected**: an out-of-range effort snaps into range, and `none` is honored when the
model has an off switch or snaps to the floor when it does not. When the requested effort is adjusted,
the request log records both values under `metadata.reasoning` (`requested`, `effective`, `clamped`) so
the clamping is observable.
...
| `levels` | The effort levels the model accepts, as points on the ladder `none < minimal < low < medium < high < xhigh`. Include `"none"` to declare a literal off switch; omit it for a model that **always reasons** (a `none` request then snaps to the lowest level). E.g. `["none","low","medium","high"]`, or `["none","high"]` for a binary toggle. There is no separate `canDisable` flag. |
...
```jsonc
"reasoning": { "kind": "openai_effort", "levels": ["none","low","high","xhigh"], "upstreamEffortMap": { "xhigh": "max" } }
```

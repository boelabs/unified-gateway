...
and user agents are fingerprinted rather than stored verbatim. If `OBSERVABILITY_ENCRYPTION_KEY` is
configured, the gateway captures 1% of healthy successes by default and every degraded, incomplete,
blocked, failed, or cancelled operation. Encryption and fingerprinting use separate HKDF-derived
...
also update the sample access timestamp. If the key is absent, metadata remains available and the
gateway never falls back to plaintext.
...
`/health`, the observability summary, and structured JSON logs remain sufficient for the initial
operating model.

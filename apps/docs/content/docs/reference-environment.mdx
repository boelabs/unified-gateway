...
| `LIVE_TRANSCRIPTION_MAX_CONNECTIONS` | `1000` | Maximum live transcription WebSocket connections per gateway process. |
| `LIVE_TRANSCRIPTION_MAX_CONNECTIONS_PER_KEY` | `20` | Maximum live transcription WebSocket connections for one virtual key (the master key has its own scope). |
| `LIVE_TRANSCRIPTION_MAX_SESSION_MS` | `3600000` (1h) | Hard lifetime cap for one live transcription session. |
| `LIVE_TRANSCRIPTION_MAX_EVENT_BYTES` | `4194304` (4 MiB) | Maximum size of one client WebSocket JSON event, including base64 audio. |
| `LIVE_TRANSCRIPTION_MAX_QUEUED_BYTES` | `16777216` (16 MiB) | Maximum aggregate size of client events waiting to be forwarded on one live transcription connection. |
| `LIVE_TRANSCRIPTION_MAX_BUFFERED_BYTES` | `1048576` (1 MiB) | Downstream WebSocket backpressure threshold; a client that remains above it for 30 seconds is disconnected. |
